...
const memoryClient_1 = require("./services/memoryClient");
const nameExtractionService_1 = require("./services/nameExtractionService");
...
const MEMORY_MCP_URL = process.env.MEMORY_MCP_URL ?? (() => { throw new Error('MEMORY_MCP_URL is not set in .env file'); })();
const PORT = parseInt(process.env.PORT || '3000');
class MemoryGlassesApp extends sdk_1.AppServer {
    constructor() {
        super({
        ...
        // Initialize services
        this.memoryClient = new memoryClient_1.MemoryClient(MEMORY_MCP_URL);
        this.nameExtractor = new nameExtractionService_1.NameExtractionService(OPENAI_API_KEY, OPENAI_MODEL);
        console.log('Memory Glasses App initialized');
        console.log('Services ready:');
        console.log('- Memory MCP Client');
    ...
    async onSession(session, sessionId, userId) {
        ...
        this.conversationManager = new conversationManager_1.ConversationManager(this.memoryClient, this.nameExtractor);
        // Show welcome message
        session.layouts.showTextWall("Memory Assistant Ready!\nListening for names...", {
            view: sdk_1.ViewType.MAIN,
            durationMs: 3000
        });
        ...
        session.events.onTranscription(async (data) => {
            ...
            if (data.isFinal && data.text.trim()) {
                ...
                const result = await this.conversationManager.processTranscription(speaker, data.text, true);
     ...
     */
    async processBufferedTranscripts(session, transcripts) {
        if (!this.conversationManager || transcripts.length === 0)
            return;
        ...
        try {
            ...
            for (const extracted of names) {
                ...
                if (existingPerson) {
                    // Show recognition
                    session.layouts.showTextWall(`Welcome back ${extracted.name}!`, {
                        view: sdk_1.ViewType.MAIN,
                        durationMs: 4000
                    });
                }
                    ...
                    session.layouts.showTextWall(`Nice to meet you ${extracted.name}!`, {
                        view: sdk_1.ViewType.MAIN,
                        durationMs: 4000
                    });
                }
